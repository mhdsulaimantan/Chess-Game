module Rules
    ...
    def checkmate?(last_position, board = @board)
      ...
      king_value = board.player == 'w' ? '♚ ' : '♔ '
      board.get_possible_moves(last_position).each do |move|
        move = move.split('')
        return true if board.chess_board[8 - move.last.to_i][board.row.index(move.first)] == king_value
      end
      false
    end
